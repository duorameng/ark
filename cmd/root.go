...
func PrintHelp() {
	...
	fmt.Println("  ark scan -e \"logs,*.tmp\"      # 扫描探测时排除指定目录或通配符规则并保存")
	fmt.Println("  ark board                     # 默认登船 (按秒级时间戳: {分类}-YYYYMMDD-HHMMSS)")
	fmt.Println("  ark board --auto-scan         # 登船前全自动探测最新目录结构与排除过滤，动态同步清单")
	fmt.Println("  ark board -e \"watchover,temp\" # 登船装箱时临时排除指定目录")
